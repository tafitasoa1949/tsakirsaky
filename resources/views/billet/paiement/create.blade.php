    ...
    <section class="section">
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">Payement Billet</h5>
              <!-- Vertical Form -->
              <form class="row g-3" action="{{ route('paiementInsert') }}" method="POST">
                @csrf
                <div class="col-12">
                ...
                <div class="col-12">
                    ...
                    <select class="form-select" aria-label="Default select example" name="idmode_paiment">
                        <option selected="">Operateur</option>
                        @foreach ($modePaiements as $mode)
                            <option value="{{ $mode->id }}">{{ $mode->nom }}</option>
                        @endforeach
                    </select>
                    @error('idmode_paiment')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
                </div>
                <div class="col-12">
                    <label for="inputPassword4" class="form-label">Telephone</label>
                    <input type="number" step="any" min="100" class="form-control" id="inputPassword4" name="telephone">
                    @error('telephone')
                  <div class="text-danger">{{ $message }}</div>
                  @enderror
                  </div>
                <div class="col-12">
                  ...
                  <input type="number" step="any" min="100" class="form-control" id="inputPassword4" name="montant">
                  @error('montant')
                <div class="text-danger">{{ $message }}</div>
                @enderror
                </div>
                <div class="col-12">
                  ...
                  <input type="number" step="any" min="100" class="form-control" id="inputAddress" name="reference">
                  @error('reference')
                <div class="text-danger">{{ $message }}</div>
            @enderror
                </div>
                <div class="col-12">
                    ...
                    <input type="date" step="any" min="100" class="form-control" id="inputAddress" name="date">
                    @error('date')
                  <div class="text-danger">{{ $message }}</div>
              @enderror
                  </div>
